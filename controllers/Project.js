...
var projectController = express.Router()
...
projectController.post('/form',async (req,res)=>{
    ...
    if(req.body._id){
       Project.findByIdAndUpdate(req.body._id, req.body, {new:true,lean:true}, function (error,doc) {
           res.redirect(`/dashboard?space_id=${doc.workspace._id}&project_id=${doc._id}`);
        })        
    }else{
        // Create new project
        try {
            let workspace = await Workspace.findById(req.body.space_id).lean().exec()
          ...
          res.redirect(`/dashboard?space_id=${result.workspace._id}&project_id=${result._id}`);
